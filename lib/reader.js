...
module.exports = async function (file) {
  ...
  const logHeader = `{green-bg}{white-fg}TBLReader ${version} (co3moz © MIT 2018){/}`;
  ...
  tableRows.unshift(columns.map((x, i) => typeResolve(x) + '(' + i + ')'));
  ...
  table.on('select', function (data, index) {
    ...
    let tableData = tableRows[index].map((x, i) => [i.toString(), typeResolve(columns[i]), x.toString()]);
    tableData.unshift(['id', 'type', 'value']);
